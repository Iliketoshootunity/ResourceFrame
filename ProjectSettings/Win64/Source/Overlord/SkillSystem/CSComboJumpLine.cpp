...
void UCSComboJumpLine::RespondInput(EInputStatus InputStatus)
{
	if (OwnClip)
	{
		...
		else if (Data.RespondType == EComboJumLineRespondType::BeforeCharge)
		{
			//第三种情况，蓄力动画之前的跳转条件
			if (InputStatus == EInputStatus::Relese)
			{
...
void UCSComboJumpLine::Tick(float DeltaTime)
{
	if (Data.RespondType == EComboJumLineRespondType::Charge)
	{
		if (bStartCharge)
		{
			...
			if (ClipData)
			{
				if (ChargeTimer > ClipData->ChargeLengthMax)
				{
					//跳转
					SetCanJump(true);
					return;
				}
				else if (!bMatchCharge && ChargeTimer > ClipData->ChargeLength)
				{
					bMatchCharge = true;
					OwnClip->MatchInputSucceed(this);
				}
