...
bool ComboClipTable::ReadTable(int32 nRow, int32& nCol)
{
	...
	if (!GetTableManager()->GetTabString(nRow, nCol++, ANSI_NONE, data.Path))
		return false;
	GetTableManager()->GetTabInteger(nRow, nCol++, INVALID_INDEX, data.ChantLength);
	GetTableManager()->GetTabInteger(nRow, nCol++, INVALID_INDEX, data.ChantTimeNum);
	GetTableManager()->GetTabInteger(nRow, nCol++, INVALID_INDEX, data.ChargeLength);
	GetTableManager()->GetTabInteger(nRow, nCol++, INVALID_INDEX, data.ChargeLengthMax);
